...
// Cada entrada en este array es una migración.
// NUNCA modifiques una migración ya existente.
// SIEMPRE agrega las nuevas al final con el siguiente número.
const MIGRATIONS: { version: number; sql: string }[] = [
  ...
  // Ejemplo de cómo agregar una migración futura:
  // {
  //   version: 2,
  //   sql: `ALTER TABLE productos ADD COLUMN unidad TEXT;`,
  // },
];
...
export function runMigrations(): void {
  ...
  db.execSync(`
    CREATE TABLE IF NOT EXISTS schema_migrations (
      ...
      aplicadaEn TEXT NOT NULL
    );
  `);

  // Obtener la versión más alta ya aplicada
  const row = db.getFirstSync<{ maxVersion: number }>(
    `SELECT COALESCE(MAX(version), 0) AS maxVersion FROM schema_migrations`
  );
  const versionActual = row?.maxVersion ?? 0;

  // Aplicar solo las migraciones pendientes, en orden
  const pendientes = MIGRATIONS.filter((m) => m.version > versionActual);

  for (const migracion of pendientes) {
    db.withTransactionSync(() => {
      db.execSync(migracion.sql);
      db.runSync(
        `INSERT INTO schema_migrations (version, aplicadaEn) VALUES (?, ?)`,
        [migracion.version, new Date().toISOString()]
      );
    });
  }
}